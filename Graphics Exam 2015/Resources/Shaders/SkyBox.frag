...
void main()
{
	vec2 tileSize = vec2(1.0/4.0, 1.0/4.0);
	vec2 actualTextCoord = TextureOffset + tileSize * fragmentTextureCoordinate;

	vec4 textureColor = texture(textureBuffer, actualTextCoord);
	...
	vec3 normals = fragmentNormal_cameraspace * -1;

	vec4 diffuseColor = calculateDiffuseColor(materialDiffuseColor, normals, lightDirection_cameraspace);
	vec4 specularColor = calculateSpecularColor(materialSpecularColor, normals, lightDirection_cameraspace, specularExponent);
